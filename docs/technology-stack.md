...
| Local paper text search | PostgreSQL full-text search (`tsvector` / `tsquery`, GIN, config `simple`) | [Papers search](specs/papers-search.md) against ingested `Paper`s. Search document: [Paper indexing](specs/2.2.4-paper-indexing.md). No extra search service |
