...
| Job orchestrator | Prefect | Long-running jobs (source-inform and paper briefs when wired). Compose services land with [Fulfill papers metadata](specs/06-fulfill-papers-metadata.md) ([local-development.md](local-development.md)). |
...
Today, related-paper search calls dlt sources for extract and merges in `paper_reviewer.topic_brief_generation.related_paper_search`. Prefect Compose services for source-inform (and later brief) jobs are planned with step 6; dlt→Postgres load for the search path remains planned where noted in step specs. Step-specific rules: [specs/03-related-paper-search.md](specs/03-related-paper-search.md), [specs/06-fulfill-papers-metadata.md](specs/06-fulfill-papers-metadata.md).
...
- **Prefect** — Orchestrator for long-running or multi-step jobs (source-inform first; briefs next). Compose services are added with step 6 ([local-development.md](local-development.md)). Trigger from the UI via enqueue helpers; keep business steps in flows/tasks, not in Streamlit callbacks. Progress UIs poll durable DB columns, not Prefect run state.
