...
| Job orchestrator | Prefect | Long-running jobs (source-inform and paper briefs when wired). Compose services: [local-development.md](local-development.md); job contracts: [Fulfill papers metadata](specs/06-fulfill-papers-metadata.md), [Generate paper brief](specs/07-generate-paper-brief.md). |
...
Today, related-paper search calls dlt sources for extract and merges in `paper_reviewer.topic_brief_generation.related_paper_search`. Prefect Compose services (`prefect-server`, `prefect-worker`) run with the app profile for source-inform (and later brief) jobs; dlt→Postgres load for the search path remains planned where noted in step specs. Step-specific rules: [specs/03-related-paper-search.md](specs/03-related-paper-search.md), [specs/06-fulfill-papers-metadata.md](specs/06-fulfill-papers-metadata.md).
