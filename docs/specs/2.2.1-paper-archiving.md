...
This document is the specification for **phase 2, step 2.2.1** of the Topic scope workflow in [README.md](../../README.md).

In this step, the system maps each **paper candidate** to a reusable **`Paper`** record in the database. If that paper already exists (same source handle), the step reuses the existing record. The **same Streamlit page** then enqueues `ingest_paper` for papers that still need first ingest, and shows progress for source record, full text, paper brief, and paper-brief evaluation. Domain jobs remain owned by [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md), [Generate paper brief](2.2.3-generate-paper-brief.md), and [Paper brief evaluation](2.2.4-paper-brief-evaluation.md).
...
A **Topic scope workflow** is the four-phase workflow in [README.md](../../README.md), run on one `TopicScope`. This document specifies Paper archiving on the **External sources ingestion** path for that scope.

This page does **not** implement EFetch, PMC Cloud, or the LLM. It submits `ingest_paper`, which sequences those jobs as Prefect subflows. PubMed EFetch / PMC Cloud: [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md) and [external-sources/pubmed.md](external-sources/pubmed.md). Brief drafting: [Generate paper brief](2.2.3-generate-paper-brief.md). Evaluation: [Paper brief evaluation](2.2.4-paper-brief-evaluation.md). The **paper brief** is still the result artifact of Generate paper brief.
...
- Dedicated Streamlit page for this step that auto-runs archiving, then enqueues `ingest_paper` for selected papers, then shows ingest progress.
- Record `created_paper_ids` on `PaperArchivingResult` (inserts in this call, first-seen order).
...
- Full-record fetch (e.g. PubMed EFetch) or abstract payloads inside Streamlit (Prefect owns those calls).
- Implementing `inform_source_record`, `inform_full_text`, or `create_paper_brief` (owned by [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md) and [Generate paper brief](2.2.3-generate-paper-brief.md)).
- Dedicated Streamlit pages for Fulfill papers metadata, Generate paper brief, or Paper brief evaluation.
...
flowchart TB
  ...
  archive -->|"new or never-ingested papers"| ingest
  archive --> topic
```
...
2. **Paper archiving** (this specification) creates or reuses `Paper` records from every search candidate. The dedicated Streamlit page auto-runs this step when prerequisites exist, then enqueues `ingest_paper` for selected papers, then polls durable statuses.
3. **`ingest_paper`** fills source record then full text, then drafts a paper brief when full text succeeded, then scores that brief when the brief succeeded — see [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md#ingest_paper-orchestrator), [Generate paper brief](2.2.3-generate-paper-brief.md), and [Paper brief evaluation](2.2.4-paper-brief-evaluation.md).
4. **Topic brief generation** uses those briefs.

## Public API

Package path: `paper_reviewer.topic_scope.paper_archiving` — see [project-structure.md](../project-structure.md).

```text
archive_papers(session, candidates) -> PaperArchivingResult
enqueue_ingest_papers(session, archiving_result, submit_ingest) -> PaperIngestEnqueueResult
```
...
PaperIngestEnqueueResult
  ...
  skipped_already_existed: list[int]
```
...
| `created_paper_ids` | Primary keys of rows **inserted** in this call, first-seen order. Reuse and DOI-update of an existing row are not listed. In-run duplicate identity of a new row lists the id once. |
...
| Topic intake Submit | Wipe the entire UI session, then write the new Topic scope identity. After a new search, the archiving page can run again. |
| Search external sources re-runs | Search clears `paper_archiving_result` and `paper_ingest_enqueue_result` so this page re-runs on the latest `candidates` set. |
...
| After successful archive, no ingest enqueue cache | Enqueue `ingest_paper` for selected papers; store enqueue result. |
| Paper inserted in this call | Submit `ingest_paper`. |
| Paper reused and `source_record_status` is `not_started` | Submit `ingest_paper` (never-ingested row). |
| Paper reused with any terminal source-record or full-text status | Do not auto-submit. Show stored statuses (Skipped (already done) when succeeded before this enqueue). |
| Skip or error candidate | Not in `papers`; no job. |
...
Streamlit is presentation only ([technology-stack.md](../technology-stack.md)). Domain work for create/reuse stays in `archive_papers`. Heavy ingest runs in Prefect via `ingest_paper`. The page owns session keys, the DB commit for archiving, enqueue, and display.
...
| `paper_ingest_enqueue_result` | `PaperIngestEnqueueResult` | Optional cache that ingest enqueue was submitted for this session (not progress truth). |
...
**Invalidate on new intake:** When Topic intake Submit starts a new `TopicScope`, clear the **entire** UI session, then write the new `topic_statement` and set the Topic scope id in the URL. Topic scopes must not reuse another workflow’s session state.

**Invalidate on search external sources re-run:** When search external sources runs again (cache miss or Topic scope key mismatch), clear `paper_archiving_result` and `paper_ingest_enqueue_result` so this page re-runs on the latest `candidates` set (search owns that clear). Rule: re-run step N → clear steps N+1….

Does **not** delete durable global `Paper` or `PaperBrief` rows.
...
3. If prerequisites exist and there is no cached result → run inside `session_scope` with a spinner:
   ...
   - clear `paper_ingest_enqueue_result` so ingest enqueue runs for this new archive set
4. On unexpected failure (session unusable or commit failure) → show an error; do **not** store a partial result.
5. Empty `candidates` → still treat as success: store/display empty `PaperArchivingResult` (`papers=[]`, `skipped=[]`, `errors=[]`, `created_paper_ids=[]`) with an explanatory caption. Prefer calling `archive_papers` (no-op) rather than inventing a parallel empty path.
6. After a stored archive result, if `paper_ingest_enqueue_result` is missing → call `enqueue_ingest_papers` (reload statuses from the DB); store the enqueue result. Do not submit again on later reruns.
7. Show archive summary/sections, then a **Progress** fragment that reloads each archived paper from the DB (source-record, full-text, brief, and evaluation statuses). Poll while any submitted paper still has work in progress (`not_started` on an aspect that the job will write). When a brief succeeded, wait until `evaluation_status` is terminal (`succeeded` or `failed`).
8. On each progress row, when both `source_record_status` and `full_text_status` are terminal, show a secondary **Regenerate** button. Click submits `ingest_paper` for that paper. Unique Streamlit key per `paper_id`.
9. When every paper in the set is terminal for ingest progress (source and full text terminal; brief terminal or full text not `succeeded`; evaluation terminal when the brief succeeded), show a summary and `st.page_link` to **Topic scope**.

Do **not** add a re-run / “Archive again” button in v1. Do **not** run EFetch, Cloud, or the LLM inside Streamlit.
...
After the archive sections, render a **Progress** list for every id in `papers` (reload from the DB):

- Title as a markdown link via `url`; optional PMC / PDF links when enrichment set them.
- Caption: DOI · source-record status · full-text status · brief status · evaluation status or mean · short error when an aspect, brief, or evaluation is `failed`.
- When evaluation **succeeded** and `evaluation_score` is set, show the mean with two decimals (for example `evaluation 4.25`). Show that fragment only when a brief succeeded. Do **not** list the four G-Eval criteria. Do **not** show a number unless evaluation succeeded (`evaluation Fulfilling` while running; `evaluation Failed` on failure).
- When `PaperBrief.error_message` or `evaluation_error_message` includes an `Assistant output:` dump, show only the validation line in the caption and put the raw text in an **Assistant output** expander.
- Do **not** show `prompt_tokens`, `completion_tokens`, or `total_tokens`.

Status display labels (map each aspect independently; brief uses the same set plus the full-text gate):

| Durable signal | Display |
| --- | --- |
| `not_started` after enqueue for that aspect | Fulfilling |
| `succeeded` | Succeeded |
| `failed` | Failed |
| `unavailable` | Unavailable |
| Already `succeeded` before this enqueue (not submitted) | Skipped (already done) |
| Brief gated by full text `failed` or `unavailable` | Blocked (no full text) |
...
- **Exit:** When ingest progress is terminal for the archived set, link to **Topic scope** (pass the Topic scope id in `query_params`).
- **Input:** The archiving page consumes `SearchExternalSourcesResult.candidates` only.

## Auto-enqueue selection

`enqueue_ingest_papers` walks `PaperArchivingResult.papers` in first-seen order. For each paper, reload the row from the DB (skip missing ids):

| Condition | Action |
| --- | --- |
| `id` is in `created_paper_ids` | Submit `ingest_paper`. |
| Reused row and `source_record_status` is `not_started` | Submit `ingest_paper`. |
| Otherwise | Do not submit; append to `skipped_already_existed`. |

Empty `papers` → empty enqueue result; no jobs.

Enqueue is unbounded: Streamlit submits one `ingest_paper` run per selected paper. Prefect holds extra runs until a slot is free. The execution cap is owned by [local-development.md](../local-development.md) (`prefect-worker`).

`ingest_paper` always forces. Freeze for reused succeeded papers is this selection, not a `force=false` flag.

## Orchestration boundary

| Responsibility | Owner |
| --- | --- |
| Map candidates → create-or-reuse / skip `Paper`; `created_paper_ids`; ingest enqueue selection | `paper_reviewer.topic_scope.paper_archiving` (`archive_papers`, `enqueue_ingest_papers`) |
...
| Render page, session keys, auto-run + commit, ingest enqueue, progress display | `paper_reviewer.ui.paper_archiving` |
| PubMed EFetch / full text (PMC Cloud); `PaperAspectStatus`; `ingest_paper` steps 1–2 | [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md); [external-sources/pubmed.md](external-sources/pubmed.md) |
...
| Pydantic `Paper`, `PaperArchivingResult`, skip/error types, `PaperIngestEnqueueResult` | `paper_reviewer.schemas.topic_scope` |
| ORM `Paper` + thin create/get | `paper_reviewer.models.paper` |

This document is the **behavior contract** for domain logic and for the Streamlit page. Domain package, schemas, ORM, and migrations may already exist; ingest job internals stay in 2.2.2 / 2.2.3 / 2.2.4.
...
- Empty list → empty result (`created_paper_ids=[]`).
- Insert new identities; assert uppercase DOI storage; id in `created_paper_ids`.
- Reuse by `(source_id, source_uid)`; assert non-DOI fields unchanged; id **not** in `created_paper_ids`.
- DOI A→B when B free → stored DOI becomes B; id **not** in `created_paper_ids`.
...
- Duplicate input identity → one `papers` entry and one `created_paper_ids` entry; first-seen order.
- Savepoint failure on one candidate → that candidate in `errors`; others still in `papers`.

**Domain (`enqueue_ingest_papers`):**

- Inserted ids submitted; reused terminal papers skipped.
- Reused `source_record_status=not_started` submitted.
- Empty `papers` → empty enqueue result.
- Missing paper id dropped.

**UI slice** (no Streamlit widget assertions per [tdd.md](../tdd.md)):

- `tests/ui/test_navigation.py`: page registered with key `paper_archiving`, title **Paper archiving**, render callable `render_paper_archiving`, `url_path` `paper-archiving`. Fulfill papers metadata, Generate paper brief, and Paper brief evaluation pages are **not** registered.
- Pure helpers (skip reason → display label, paper caption lines, aspect/brief/evaluation progress labels, `paper_ingest_row_is_terminal`, `may_submit_ingest_paper`, brief error split) live in `tests/ui/test_paper_archiving.py` and are unit-tested without Streamlit.

## Non-goals (v1)

Do not do this work in the Paper archiving v1 slice:

- Call EFetch or any full-record API from Streamlit.
- Implement LLM drafting inside Streamlit.
...
- Show create vs reuse vs DOI-update labels per paper on the archived-paper cards (enqueue uses `created_paper_ids`, not card labels).
