...
In this step, the system maps each **paper candidate** to a reusable **`Paper`** record in the database. If that paper already exists (same source handle), the step reuses the existing record. The **same Streamlit page** then enqueues `ingest_paper` for papers that still need first ingest, and shows progress for source record, full text, and paper brief. Domain jobs remain owned by [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md) and [Generate paper brief](2.2.3-generate-paper-brief.md).
...
- Dedicated Streamlit pages for Fulfill papers metadata or Generate paper brief.
...
3. **`ingest_paper`** fills source record then full text, then drafts a paper brief when full text succeeded — see [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md#ingest_paper-orchestrator) and [Generate paper brief](2.2.3-generate-paper-brief.md).
...
7. Show archive summary/sections, then a **Progress** fragment that reloads each archived paper from the DB (source-record, full-text, and brief statuses). Poll while any submitted paper still has work in progress (`not_started` on an aspect that the job will write).
...
9. When every paper in the set has both aspects terminal, and each brief is terminal or full text is not `succeeded`, show a summary and `st.page_link` to **Topic scope**.
...
- Caption: DOI · source-record status · full-text status · brief status · short error when an aspect or brief is `failed`.
- When `PaperBrief.error_message` includes an `Assistant output:` dump, show only the validation line in the caption and put the raw text in an **Assistant output** expander.
...
This document is the **behavior contract** for domain logic and for the Streamlit page. Domain package, schemas, ORM, and migrations may already exist; ingest job internals stay in 2.2.2 / 2.2.3.
...
- `tests/ui/test_navigation.py`: page registered with key `paper_archiving`, title **Paper archiving**, render callable `render_paper_archiving`, `url_path` `paper-archiving`. Fulfill papers metadata and Generate paper brief pages are **not** registered.
- Pure helpers (skip reason → display label, paper caption lines, aspect/brief progress labels, `may_submit_ingest_paper`, brief error split) live in `tests/ui/test_paper_archiving.py` and are unit-tested without Streamlit.
