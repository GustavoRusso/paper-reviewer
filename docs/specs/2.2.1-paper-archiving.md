...
In this step, the system maps each **paper candidate** to a reusable **`Paper`** record in the database. If that paper already exists (same source handle), the step reuses the existing record. The **same Streamlit page** then enqueues `regenerate_paper` for papers that still need first ingest, and shows progress for source record, full text, and paper brief. Domain jobs remain owned by [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md) and [Generate paper brief](2.2.3-generate-paper-brief.md).
...
- Dedicated Streamlit page for this step that auto-runs archiving, then enqueues `regenerate_paper` for selected papers, then shows ingest progress.
...
flowchart TB
  ...
  archive -->|"new or never-ingested papers"| regen
  archive --> topic
```
...
2. **Paper archiving** (this specification) creates or reuses `Paper` records from every search candidate. The dedicated Streamlit page auto-runs this step when prerequisites exist, then enqueues `regenerate_paper` for selected papers, then polls durable statuses.
3. **`regenerate_paper`** fills source record then full text, then drafts a paper brief when full text succeeded — see [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md#full-regenerate-orchestrator) and [Generate paper brief](2.2.3-generate-paper-brief.md).
...
enqueue_regenerate_papers(session, archiving_result, submit_regenerate) -> PaperIngestEnqueueResult
```
...
| After successful archive, no ingest enqueue cache | Enqueue `regenerate_paper` for selected papers; store enqueue result. |
| Paper inserted in this call | Submit `regenerate_paper`. |
| Paper reused and `source_record_status` is `not_started` | Submit `regenerate_paper` (never-ingested row). |
...
Streamlit is presentation only ([technology-stack.md](../technology-stack.md)). Domain work for create/reuse stays in `archive_papers`. Heavy ingest runs in Prefect via `regenerate_paper`. The page owns session keys, the DB commit for archiving, enqueue, and display.
...
6. After a stored archive result, if `paper_ingest_enqueue_result` is missing → call `enqueue_regenerate_papers` (reload statuses from the DB); store the enqueue result. Do not submit again on later reruns.
...
8. On each progress row, when both `source_record_status` and `full_text_status` are terminal, show a secondary **Regenerate** button. Click submits `regenerate_paper` for that paper. Unique Streamlit key per `paper_id`.
...
`enqueue_regenerate_papers` walks `PaperArchivingResult.papers` in first-seen order. For each paper, reload the row from the DB (skip missing ids):

| Condition | Action |
| --- | --- |
| `id` is in `created_paper_ids` | Submit `regenerate_paper`. |
| Reused row and `source_record_status` is `not_started` | Submit `regenerate_paper`. |
| Otherwise | Do not submit; append to `skipped_already_existed`. |

Empty `papers` → empty enqueue result; no jobs.

`regenerate_paper` always forces. Freeze for reused succeeded papers is this selection, not a `force=false` flag.

## Orchestration boundary

| Responsibility | Owner |
| --- | --- |
| Map candidates → create-or-reuse / skip `Paper`; `created_paper_ids`; ingest enqueue selection | `paper_reviewer.topic_scope.paper_archiving` (`archive_papers`, `enqueue_regenerate_papers`) |
...
| PubMed EFetch / full text (PMC Cloud); `PaperAspectStatus`; `regenerate_paper` steps 1–2 | [Fulfill papers metadata](2.2.2-fulfill-papers-metadata.md); [external-sources/pubmed.md](external-sources/pubmed.md) |
...
**Domain (`enqueue_regenerate_papers`):**
...
- Pure helpers (skip reason → display label, paper caption lines, aspect/brief progress labels, `may_submit_regenerate_paper`, brief error split) live in `tests/ui/test_paper_archiving.py` and are unit-tested without Streamlit.
