...
| `force` is true (from `ingest_paper` only) and full text `succeeded` | Rewrite `content` and overwrite the three usage columns even if brief was `succeeded` or `failed`; then `succeeded` or `failed` from this attempt. |
