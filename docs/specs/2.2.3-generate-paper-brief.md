...
4. **Paper brief evaluation** may then run as `ingest_paper` step 4 — [Paper brief evaluation](2.2.4-paper-brief-evaluation.md). This step does not run the judge.
...
| LLM / validation / DB error | Set `failed` + `error_message`. Clear `content`, the three usage columns, and the evaluation columns. On `PaperBriefContent` parse failure, `error_message` includes the validation or extract error and the capped assistant text. Other failures (timeout, missing key) do not dump assistant text. |
