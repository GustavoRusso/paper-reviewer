...
This document is the specification for **phase 2, step 1** of the Topic scope workflow in [README.md](../../README.md).
...
Stack context: [technology-stack.md](../technology-stack.md) (dlt extract + Pydantic; Prefect runs source-record / full-text / brief jobs in Compose — not this search step). Package paths: `paper_reviewer.ingest` (sources), `paper_reviewer.topic_scope.search_external_sources` (orchestration / merge) — see [project-structure.md](../project-structure.md).
...
A **Topic scope workflow** is the four-phase workflow in [README.md](../../README.md), run on one `TopicScope`. This document specifies only phase 2 step 1 (Search external sources) for that scope.
...
- Local-database Papers search / References selection ([Papers search](papers-search.md), [References selection](3-references-selection.md)).
...
| `pub_date`       | Full publication date when the source supplies year, month, and day; otherwise null                      |
...
`search_external_sources` in `paper_reviewer.topic_scope.search_external_sources` accepts a `TopicAnalysisResult` directly (DB-loaded or an injected fixture). That is not the Streamlit input.
...
| Concern | Rule |
| --- | --- |
| Owner | This workflow (`paper_reviewer.topic_scope.search_external_sources`). |
...
- `paper_reviewer.topic_scope.search_external_sources` accepts a `TopicAnalysisResult`, converts internally to `SearchCriteria` when needed, runs registered sources, and **merges** results into one global list (see merge rules below).
...
6. **Sort** the global list: **year descending**, then **date descending** when `pub_date` is present; candidates with missing `published_year` last; within the same year, candidates with missing `pub_date` last; tie-break on uppercase `doi`.
...
| `retmax` truncates | Candidates limited accordingly; when the facet value is null, PubMed defaults to **200** (ESummary JSON cap **500**) |
...
4. **Cache miss or key mismatch** — If there is no cached result, or the cached key does not match the URL key: ignore the old cache; clear downstream ingest session caches for the old run (same cascade idea as [Paper archiving](2.2.1-paper-archiving.md)); reload facets from the DB; call `search_external_sources` with a spinner; store the new result **together with** the current `topic_scope_key`. Fail-soft `source_runs` as in Behavior.
...
Topic intake Submit still wipes the whole session — [Paper archiving](2.2.1-paper-archiving.md). After that wipe, this page searches again from DB facets for the new URL key.
