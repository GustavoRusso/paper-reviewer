...
In this step, the system searches registered **external sources** for related papers and produces a global list of **paper candidates** for [Paper archiving](05-paper-archiving.md).
...
| Term | Meaning |
| --- | --- |
| **Search external sources** | Paper ingestion step 2.1: search external sources with topic facets and produce `PaperCandidate`s for Paper archiving. |
...
- Prefect may later schedule these extracts; orchestration ownership stays in this workflow. Prefect Compose services today own source-record / full-text / brief jobs ([06-fulfill-papers-metadata.md](06-fulfill-papers-metadata.md)), not search external sources.
...
4. **Cache miss or key mismatch** — If there is no cached result, or the cached key does not match the URL key: ignore the old cache; clear downstream ingest session caches for the old run (same cascade idea as [Fulfill papers metadata](06-fulfill-papers-metadata.md)); reload facets from the DB; call `search_external_sources` with a spinner; store the new result **together with** the current `topic_scope_key`. Fail-soft `source_runs` as in Behavior.
