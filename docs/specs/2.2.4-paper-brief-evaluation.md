...
One judge call per brief. Temperature 0. Structured JSON (same pattern as `create_paper_brief`: `json_schema`, then validate). When `OPENAI_BASE_URL` is set, the judge also sends `reasoning_effort=none` (same as [Generate paper brief](2.2.3-generate-paper-brief.md)). If `content` is empty, the client reads a reasoning field on the assistant message and validates that text.
