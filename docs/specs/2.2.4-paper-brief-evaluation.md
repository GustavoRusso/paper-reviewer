...
**Not implemented.** This document locks the **behavior contract** (criteria, JSON, `PaperBrief` columns, Prefect job). There is no domain package, prompt file under `src/`, Alembic migration, Prefect job, or Streamlit page yet.
...
- Lock the in-house G-Eval-style pointwise judge: score the **whole brief** once on four criteria, JSON shape, and overall mean.
- Persist the judge artifact and `evaluation_score` **on** `PaperBrief` (not inside `content`, not a separate table).
- State that evaluation is **advisory**: it does not change `PaperBrief.status` or `content` and does not block [Topic brief generation](4-topic-brief-generation.md).
- Name the future package, prompt file, and Prefect job so implementation has a single owner.

### Out of scope (this document)

- Any application code, tests, Alembic migration, or Streamlit page (this docs slice).
- Drafting or rewriting `PaperBrief.content` — owned by [Generate paper brief](2.2.3-generate-paper-brief.md).
- Viewing stored paper-brief prose or evaluation scores — owned by [Paper brief](paper-brief.md) (reader does not show scores in v1).
...
This spec owns the criterion list, evaluation steps, and rubric until `paper_brief_evaluation_template.md` exists. After that file exists, this spec points at it and does not copy prompt prose. Criteria match the generator grounding rules in [`paper_brief_template.md`](../../src/paper_reviewer/topic_scope/generate_paper_brief/paper_brief_template.md). Do not copy that generator prompt into this document.

Apply all four criteria **once to the whole brief**.
...
| `completeness` | The brief matches the generator contract: required fields filled (summary = core takeaway; objective = gap + goal; `key_findings` = primary results); optional fields filled only when the text supports that aspect. An empty optional field is correct when the text does not support it. A missing required field, or an optional field invented when the text has no such content, is a completeness failure. |
| `conciseness` | Shape matches the template (summary one or two sentences; `key_findings` typically two or three items, no table dump; short labels for `study_type` and similar). |
...
### Evaluation steps (locked until the prompt file exists)

The judge follows these steps. Do not generate a new step list on each paper.
...
1. Read the generator field contract (ids and required flags from the brief template front matter).
2. Check that required fields are filled and match that contract.
3. Check optional fields: filled only when the text supports that aspect; empty is correct when the text does not support it.
4. Assign a score from 1 to 5.

**`conciseness`**

1. Check that the brief is short and matches the template shape.
2. Assign a score from 1 to 5.

**`topic_agnostic`**

1. Check that the brief describes the article, not a user research topic.
2. Assign a score from 1 to 5.

Each criterion: `reasoning` (step-by-step; may name a specific brief field) plus integer **`score` 1–5** (1 worst, 5 best).
...
If the generator template front matter adds or removes a field, update the completeness steps (and the future judge prompt) in the **same** change. Do not add per-field scores.
...
One judge call per brief. Temperature 0. Structured JSON (same pattern as `create_paper_brief`: `json_schema`, then validate).
...
Add to the global `PaperBrief` row. Do **not** put them inside `content`. Do **not** add a separate evaluation table. [Generate paper brief](2.2.3-generate-paper-brief.md) owns `status`, `content`, `error_message`, and the three generate-brief usage columns.
...
| `evaluation_error_message` | No | Set when `evaluation_status=failed`. Cleared on evaluation `succeeded`. On judge JSON parse failure, include the validation or extract error plus the raw assistant text (capped at 8000 characters) after an `Assistant output:` marker (same diagnose pattern as brief `error_message`). |
...
## Public API and Prefect entrypoints (contract; not implemented)
...
## Prefect job behavior (contract; not implemented)
...
| Brief `succeeded`, evaluation `not_started` (or no prior run) | Call judge; log request/response/usage like `create_paper_brief`; persist artifact + score; set evaluation `succeeded`; clear `evaluation_error_message`. |
| LLM / validation / DB error | Set evaluation `failed` + `evaluation_error_message`. Clear `evaluation` and `evaluation_score`. Brief `status`/`content` unchanged. |
...
There is **no** dedicated Streamlit page for this step. Do **not** register `paper_brief_evaluation` in `build_app_pages()`. Do **not** show evaluation scores on [Paper brief](paper-brief.md) in v1. Do **not** add a stepper entry until a page exists (same as [Paper indexing](2.2.5-paper-indexing.md) today).
...
| --- | --- |
| Domain package | `paper_reviewer.topic_scope.paper_brief_evaluation` |
| Prompt | `paper_brief_evaluation_template.md` (YAML front matter for the four criterion ids, the locked evaluation steps, and the judge system prompt), same pattern as the brief template. Field ids and required flags come from the generator template (completeness only). When that file exists, stop copying the rubric into this spec; this spec then points at that file. |
| Prefect | Idempotent `evaluate_paper_brief`. `ingest_paper` step 4 with `force=true` after a succeeded brief. Fail-soft per paper. |
| Persistence | Four columns on `PaperBrief` (this document). |
| UI | No stepper entry until a page exists. Reader does not show scores in v1. |

Do **not** change [project-structure.md](../project-structure.md) until the package exists.
...
**UI slice:** `tests/ui/test_navigation.py` must **not** register a Paper brief evaluation page. Paper brief reader tests must **not** require evaluation scores.
