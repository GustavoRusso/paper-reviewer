...
| Field             | Description                                      |
| ----------------- | ------------------------------------------------ |
| `strategy_id`     | Which search strategy produced the hit           |
...
| ------------------------------------ | -------- | --------------------------------------------------------------------------------------- |
| `strategies`                         | Yes      | One or more named strategies; each may yield candidates tagged with `strategy_id`       |
| `strategies[].id`                    | Yes      | Stable id used as `strategy_id` on candidates                                           |
| `strategies[].label`                 | Yes      | Human-readable label for UI / logs                                                      |
| `strategies[].intent`                | No       | Free-text note for humans / later analysis                                              |
| `strategies[].concepts`              | No       | Primary topic terms                                                                     |
| `strategies[].synonyms`              | No       | Alternate terms for the same ideas                                                      |
| `strategies[].date_from` / `date_to` | No       | Inclusive date bounds when set                                                          |
| `strategies[].filters`               | No       | Generic filter bag; each source interprets what it supports                             |
| `strategies[].retmax`                | No       | Max hits requested per strategy per source                                              |
| `source_overrides`                   | No       | Map `source_id` → opaque payload defined by that source’s spec (fixtures / power tests) |


The workflow passes each strategy (plus any matching override) into each registered source adapter. Compilation to a concrete API query is defined only in the source’s paper-sources doc.
...
3. When merging duplicates, keep one canonical candidate; retain provenance that multiple strategies/sources hit the same paper when useful for triage metadata (implementation detail).
4. Tag each candidate with `strategy_id` and `source_id`.
...
| `source_runs[]` | Per source: `source_id`, status (`ok` / `error` / `empty`), hit counts, compiled queries or strategy ids, error message if any |
...
| Case                                       | Expected                                                                                         |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| Empty `strategies`                         | Empty `candidates`; metadata notes no strategies                                                 |
| Strategy yields zero hits from a source    | That strategy contributes nothing; other strategies/sources continue                             |
