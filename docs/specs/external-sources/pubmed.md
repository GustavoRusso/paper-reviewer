...
5. If `text_url` is present: download the `.txt` body (JATS flatten). Persist `strip()` of any **non-empty** body as `full_text_plain` (including a PMC license-block stub, for debug). Empty, spaces-only, or newline-only body is the same as no `.txt` (omit `full_text_plain`). A license-block stub contains `do not permit archiving in PMC` (case-insensitive). Pull for Open Access Subset **and** Author Manuscript when Cloud returns a non-empty `.txt`.
...
7. On no Cloud object, no `.txt`, or blank / whitespace-only `.txt` in steps 2–6: leave `full_text_plain` null; the full-text flow sets `full_text_status = unavailable`. On a non-empty license-block stub: persist the stripped body and enrichment columns; the full-text flow sets `full_text_status = unavailable` (not `succeeded`). On HTTP/parse error after in-run retries: leave `full_text_plain` null; the flow sets `failed`. Do **not** treat a Cloud miss or a blank / stub `.txt` as source-record success (source record is already a separate flow). Do **not** convert HTTP/parse errors into `unavailable`.
...
| Cloud `.txt` is a license-block stub | `unavailable`; persist stripped stub in `full_text_plain`; write enrichment columns |
