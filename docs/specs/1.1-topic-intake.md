...
This document is the specification for **phase 1, step 1** of the Topic brief generation workflow in [README.md](../../README.md).
...
`TopicBriefGeneration` / `topic_brief_generations` is a **legacy** name. Specs and new code use `TopicScope` / `topic_scopes`.

## Topic brief generation

A **Topic brief generation** is the four-phase workflow in [README.md](../../README.md), run on one `TopicScope`. This document specifies only phase 1 step 1 (Topic intake) for that scope.
...
- Dedicated Streamlit **Topic intake** page. After a successful persist, **switch** to Topic analysis with `topic_scope_public_id` in the URL.
...
- Related-paper search, triage, paper archiving, fulfill papers metadata, generate paper brief, topic brief drafting.
- Prefect.
...
Package path: `paper_reviewer.topic_brief_generation.topic_intake` — see [project-structure.md](../project-structure.md).
...
| `start_topic_scope_from_topic_intake` | Call `accept_topic_intake`, then insert a `TopicScope` with that `topic_statement`. Always **insert**. Flush so `id` / `public_id` / `created_at` exist on the returned row. **Caller owns commit.** |

Replace today’s `start_topic_brief_from_topic_intake` (and `create_topic_brief_generation`) with these names.

Pydantic types live under `paper_reviewer.schemas.topic_brief_generation.topic_intake`.
...
| `public_id` | Yes | UUID. Unique. Public id in the URL query `topic_scope_public_id` ([ui-style.md](../ui-style.md#topic-scope-public-id-in-the-url)). |
| `topic_statement` | Yes | The accepted `TopicStatement.text`. |
| `created_at` | Yes (DB) | Server timestamp when the row was created. |

Table name: `topic_scopes` (legacy table `topic_brief_generations`).
...
- Confirmed papers from Paper search
...
3. On Submit:
   ...
   - Success → inside the same callback, in this order:
     ...
     3. Set `topic_scope_public_id` so the **Topic analysis** URL keeps it ([ui-style.md](../ui-style.md#topic-scope-public-id-in-the-url)).
...
The public id lives in the **URL**, not in session ([ui-style.md](../ui-style.md#topic-scope-public-id-in-the-url)).
...
| --- | --- |
| Validate statement; insert `TopicScope` | `paper_reviewer.topic_brief_generation.topic_intake` |
| Pydantic `TopicStatement` | `paper_reviewer.schemas.topic_brief_generation.topic_intake` |
| ORM `TopicScope` + thin create/get/list | `paper_reviewer.models.topic_brief_generation` |
...
- `start_topic_scope_from_topic_intake` inserts `TopicScope`; `public_id` is a UUID; `topic_statement` matches accepted text.
...
- Run Topic analysis or related-paper search on this page.
