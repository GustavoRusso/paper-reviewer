...
In this step, the system searches registered **paper sources** for related papers and produces a global list of **paper candidates** for [Paper archiving](05-paper-archiving.md).

Entry is the [Paper ingestion](2-paper-ingestion.md) landing, not Topic analysis. Facets come from the current `TopicScope` in the database — [Topic analysis](1.2-topic-analysis.md). Do **not** auto-run this page from Topic analysis.
...
| Term | Meaning |
| --- | --- |
| **Related-paper search** | Paper ingestion step 2.1: search paper sources with topic facets and produce `PaperCandidate`s for Paper archiving. |
| **`PaperCandidate`** | Normalized search hit from a paper source. Shape owned by this spec. |
| **`RelatedPaperSearchResult`** | Search output: `candidates`, `source_runs`, optional `notes`. |

## Topic brief generation

A **Topic brief generation** is the four-phase workflow in [README.md](../../README.md), run on one `TopicScope`. This document specifies only phase 2 step 1 (Related-paper search) for that scope.
...
- Run extract via **dlt** for each registered paper source.
...
- [Paper archiving](05-paper-archiving.md) and later ingest steps.
- [Fulfill papers metadata](06-fulfill-papers-metadata.md) (EFetch / PMC Cloud) or [Generate paper brief](07-generate-paper-brief.md) (creates **paper brief** results).
- Adding new paper sources beyond registering them here.
- Loading candidates into Postgres as `Paper` rows ([Paper archiving](05-paper-archiving.md) owns that).
...
`search_related_papers` in `paper_reviewer.topic_brief_generation.related_paper_search` accepts a `TopicAnalysisResult` directly (DB-loaded or an injected fixture). That is not the Streamlit input.
...
- The contract to [Paper archiving](05-paper-archiving.md) is the global `PaperCandidate` list (plus `source_runs` metadata). Paper archiving consumes every candidate from this list.
...
2. **Drop** any candidate whose `doi` is missing or blank after strip. [Paper archiving](05-paper-archiving.md) never sees no-DOI hits.
...
Primary deliverable for [Paper archiving](05-paper-archiving.md): `candidates`. `source_runs` supports debugging and UI status.
...
| Normal UI path: scope has no facet rows | Do **not** call `search_related_papers`. Show the guard (links to Topic analysis and Topic scope). |
...
| All hits lack DOI | Empty `candidates` ([Paper archiving](05-paper-archiving.md) then gets empty success) |
...
| Key | Type | Role |
| --- | --- | --- |
| `related_paper_search_result` | `RelatedPaperSearchResult` | Cached search **output** for Paper archiving handoff. Not a source of facets. |
| `related_paper_search_topic_scope_key` | UUID string | Topic scope key that the cached result belongs to. Must match the URL key before reuse. |
...
3. **Cache match** — If session has `related_paper_search_result` **and** `related_paper_search_topic_scope_key` equals the URL key: show the cached result; do not search again.
4. **Cache miss or key mismatch** — If there is no cached result, or the cached key does not match the URL key: ignore the old cache; clear downstream ingest session caches for the old run (same cascade idea as [Fulfill papers metadata](06-fulfill-papers-metadata.md)); reload facets from the DB; call `search_related_papers` with a spinner; store the new result **together with** the current `topic_scope_key`. Fail-soft `source_runs` as in Behavior.
...
Topic intake Submit still wipes the whole session — [Fulfill papers metadata](06-fulfill-papers-metadata.md). After that wipe, this page searches again from DB facets for the new URL key.

## Testability

- Inject a `TopicAnalysisResult` JSON fixture into `search_related_papers` without running Topic analysis (domain / test path).
...
- Assert the UI path does not read facets from session; cache reuse requires a matching `related_paper_search_topic_scope_key`.
