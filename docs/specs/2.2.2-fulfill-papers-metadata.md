...
4. **Topic brief generation** consumes succeeded `PaperBrief` rows and cites papers in prose.
...
Leaf flow signatures are `(paper_id, doi, force=False)` for `inform_source_record`, `inform_full_text`, and `create_paper_brief`. `ingest_paper` stays `(paper_id, doi)` and always passes `force=True` into those subflows. Streamlit never submits `inform_*`. Served leaf deployments keep default `force=False`, so a standalone Prefect UI run stays idempotent.
...
- Parent flow calls `inform_source_record`, `inform_full_text`, and (when full text succeeded) `create_paper_brief` as subflows with `force=True`.
