...
`indexing.keywords` is the v1 source for [Papers search](papers-search.md). Do not promote that list to a typed query column here. [Paper indexing](2.2.4-paper-indexing.md) generates `keywords_tsv` from that JSON path.
...
3. If `full_text_status = succeeded`, call `create_paper_brief` with force rewrite — contract: [Generate paper brief](2.2.3-generate-paper-brief.md). If full text is not `succeeded`, do not draft or rewrite a brief.
...
| Domain enqueue + aspect inform helpers | `paper_reviewer.topic_scope.fulfill_papers_metadata` |
