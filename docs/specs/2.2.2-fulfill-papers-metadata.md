...
**Next step:** [Generate paper brief](2.2.3-generate-paper-brief.md) creates a global **paper brief** only when full text is `succeeded`. Do not draft briefs in this step.
...
- Creating **paper briefs** (`PaperBrief`) or running `create_paper_brief` from this page — owned by [Generate paper brief](2.2.3-generate-paper-brief.md). Page 7 enqueues the brief flow only.
...
3. **Generate paper brief** drafts a global `PaperBrief` only when `full_text_status` is `succeeded` — see [Generate paper brief](2.2.3-generate-paper-brief.md).
...
The user starts one run with a per-paper **Regenerate** button on the Fulfill papers metadata page (and the same button on [Generate paper brief](2.2.3-generate-paper-brief.md)). The button submits `regenerate_paper` (fire-and-forget). Streamlit does not call EFetch, Cloud, or the LLM.
...
- **Exit:** When both aspects are terminal for the set, link to [Generate paper brief](2.2.3-generate-paper-brief.md).
...
| Domain enqueue + aspect inform helpers | `paper_reviewer.topic_scope.fulfill_papers_metadata` (no domain `regenerate_paper`; the Prefect parent sequences subflows) |
...
- Create or draft `PaperBrief` rows from page 6 auto-enqueue ([Generate paper brief](2.2.3-generate-paper-brief.md)). The **Regenerate** button submits `regenerate_paper`, which may rewrite a brief.
