...
- `inform_source_record` / `inform_full_text` (default skip) and `ingest_paper` (always force).
...
### Default skip rules (`inform_*` and `create_paper_brief`)
...
| Rule | Behavior |
| --- | --- |
| Independent aspect flows | Each aspect has its own flow. `ingest_paper` calls them as Prefect subflows. Standalone Prefect UI runs of `inform_*` use default skip. |
...
| Aspect results (`InformSourceRecordResult`, `InformFullTextResult`) | Leaf flows | Once per Prefect run | Flow/tests assert outcome; durable state is already on `Paper`. UI polls enums. |
...
Leaf flows are **idempotent by default** (skip rules above). The documented exception is `ingest_paper`.
...
| `PaperAspectStatus` enum; `source_record_status` / `full_text_status`; `ingest_paper` steps 1–2 | This document |
...
| Prefect flows | `paper_reviewer.flows` (`inform_source_record`, `inform_full_text`, `ingest_paper` parent) |
...
- Progress helpers live with Paper archiving (`tests/ui/test_paper_archiving.py`). `tests/ui/test_navigation.py` must **not** register a Fulfill papers metadata page. `tests/flows/test_serve.py` must **not** serve `fulfill_paper_metadata`.
