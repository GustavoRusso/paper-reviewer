...
This document is the specification for **phase 2, step 2.2.2** of the Topic scope workflow in [README.md](../../README.md).

In this step, the system fills two **global** aspects of each archived **`Paper`**: the **source record** (fuller publication details from the external source) and **full text** (article body when a full-text source can supply it). Each aspect has its own Prefect flow and its own stored status. Streamlit does **not** have a dedicated page for this step. [Paper archiving](2.2.1-paper-archiving.md) enqueues `ingest_paper`, which runs these aspects then (when full text succeeded) the paper brief and paper-brief evaluation.

**Paper brief:** [Generate paper brief](2.2.3-generate-paper-brief.md) owns `PaperBrief` and `create_paper_brief`. Do not draft briefs inside `inform_source_record` or `inform_full_text`.

This document owns `PaperAspectStatus` and the two `Paper` status columns. Brief status lives on `PaperBrief` ([Generate paper brief](2.2.3-generate-paper-brief.md)) and reuses the same enum.
...
| **`fulfill_paper_metadata`** | Prefect orchestrator: runs source record then full text for one paper, with **default skip rules** (does not unfreeze). Served for Prefect console / ops. Streamlit does not auto-enqueue this flow. |
| **`ingest_paper`** | Parent Prefect flow: may unfreeze `succeeded` and retry `failed` / `unavailable`, then rewrite the paper brief and re-run paper-brief evaluation. Calls `inform_source_record`, `inform_full_text`, and (when full text succeeded) `create_paper_brief` then `evaluate_paper_brief` as subflows with `force=true`. Auto-enqueued from Paper archiving for selected papers; also started from a per-paper **Regenerate** button on that page. See [ingest_paper orchestrator](#ingest_paper-orchestrator). |
| **Fulfill papers metadata** | Workflow **step** (this document) that owns source-record and full-text jobs and statuses. Progress UI: [Paper archiving](2.2.1-paper-archiving.md). |
...
A **Topic scope workflow** is the four-phase workflow in [README.md](../../README.md), run on one `TopicScope`. This document specifies **Fulfill papers metadata** on the External sources ingestion path for that scope.

Paper archiving (create/reuse `Paper` without EFetch) is owned by [Paper archiving](2.2.1-paper-archiving.md). PubMed EFetch request parameters, PMCID extraction, and PMC Cloud full-text details are summarized here and detailed for PubMed in [external-sources/pubmed.md](external-sources/pubmed.md).
...
- `inform_source_record` / `inform_full_text` / `fulfill_paper_metadata` (default skip) and `ingest_paper` (always force).
...
- On full-text success (PubMed): store usable `full_text_plain` from Cloud `.txt` (`strip()` of the body; not empty; not a PMC license-block stub), plus `pmcid_version`, `is_open_access`, `pmc_article_url`, and `open_access_pdf_url` when present. On a Cloud hit with a non-empty license-block stub, still store the stripped body and enrichment columns, but set `full_text_status = unavailable`.
- Set `failed` or `unavailable` per aspect (tables below). Optional per-aspect error message when `failed`.
- `ingest_paper` as the only force path (auto-enqueue selection and **Regenerate** button live on Paper archiving).

### Out of scope (v1)

- [Paper archiving](2.2.1-paper-archiving.md) create/reuse rules or its UI (including ingest progress and auto-enqueue selection).
- Creating **paper briefs** (`PaperBrief`) inside `inform_*` — owned by [Generate paper brief](2.2.3-generate-paper-brief.md). `ingest_paper` may call `create_paper_brief`.
- Scoring paper briefs — owned by [Paper brief evaluation](2.2.4-paper-brief-evaluation.md). `ingest_paper` may call `evaluate_paper_brief` as step 4.
...
- Auto-retry of `failed` or `unavailable` except via `ingest_paper`.
- Overwrite of `succeeded` except via `ingest_paper`.
...
1. **Paper archiving** yields `PaperArchivingResult.papers` (create or reuse) and enqueues `ingest_paper` for selected papers — see [Paper archiving](2.2.1-paper-archiving.md).
2. **Fulfill papers metadata** (this specification) owns `inform_source_record` then `inform_full_text` so each paper gets a terminal `source_record_status` and then a terminal `full_text_status` (unless skipped).
3. **Generate paper brief** drafts a global `PaperBrief` only when `full_text_status` is `succeeded` — see [Generate paper brief](2.2.3-generate-paper-brief.md). After archive, that call is `ingest_paper` step 3.
4. **Paper brief evaluation** scores that brief when step 3 succeeded — see [Paper brief evaluation](2.2.4-paper-brief-evaluation.md). After archive, that call is `ingest_paper` step 4.
5. **Topic brief generation** consumes succeeded `PaperBrief` rows and cites papers in prose. It does not wait for evaluation.

## `PaperAspectStatus`

One enum for every paper aspect (source record, full text, `PaperBrief.status`, and `PaperBrief.evaluation_status`).
...
### Default skip rules (`fulfill_paper_metadata` and `create_paper_brief`)
...
**Only** [`ingest_paper`](#ingest_paper-orchestrator) may unfreeze `succeeded` and retry `failed` / `unavailable`.

## Selection rules (`fulfill_paper_metadata`)

These rules apply when an operator submits `fulfill_paper_metadata` (Prefect console / ops). Streamlit auto-enqueue uses `ingest_paper` selection in [Paper archiving](2.2.1-paper-archiving.md) instead.
...
Domain package: `paper_reviewer.topic_scope.fulfill_papers_metadata` — see [project-structure.md](../project-structure.md).

Prefect flows (names are the contract): `paper_reviewer.flows`

```text
inform_source_record(paper_id, doi, force=False) -> InformSourceRecordResult
inform_full_text(paper_id, doi, force=False) -> InformFullTextResult
...
ingest_paper(paper_id, doi) -> IngestPaperResult
```
...
| Entrypoint | Role |
| --- | --- |
| `inform_source_record` | Load `Paper` by id. Apply default skip unless `force` is true. Else fetch the fuller source record **for that one paper** (PubMed: one PMID per call via dlt EFetch resource), write payload columns, set `source_record_status`, commit (flow owns persistence). |
| `inform_full_text` | Require `source_record_status = succeeded` (else do not call Cloud; see job table). Apply default skip unless `force` is true. Else fetch full text (PubMed: PMC Cloud), write enrichment columns, set `full_text_status`. |
| `fulfill_paper_metadata` | Default-skip orchestrator: run `inform_source_record` then `inform_full_text`. One Prefect run per paper. Streamlit does not auto-enqueue this flow. |
| `enqueue_fulfill_papers_metadata` | Ops helper: apply [selection rules](#selection-rules-fulfill_paper_metadata) and submit `fulfill_paper_metadata` for those paper ids. Does not unfreeze. Does not enqueue brief jobs. Not used by the Paper archiving page. |
| `ingest_paper` | Parent Prefect flow. Always forces. Calls leaf flows as subflows. See [ingest_paper orchestrator](#ingest_paper-orchestrator). |

| Rule | Behavior |
| --- | --- |
| Independent aspect flows | Each aspect has its own flow. `ingest_paper` calls them as Prefect subflows. `fulfill_paper_metadata` sequences domain helpers with default skip. |
...
```mermaid
sequenceDiagram
  participant UI as ArchivingUI
  participant Enq as enqueue_ingest_papers
  participant Pref as Prefect
  participant Ingest as ingest_paper
  participant Src as inform_source_record
  participant Ft as inform_full_text
  participant Brief as create_paper_brief
  participant Eval as evaluate_paper_brief
  participant DB as PaperRow

  UI->>Enq: PaperArchivingResult
  Enq->>Enq: created ids or not_started source
  Enq->>Pref: submit one ingest_paper per submitted id
  Enq-->>UI: PaperIngestEnqueueResult
  Note over UI: Cache enqueue result; do not EFetch here
  loop each submitted paper
    Pref->>Ingest: paper_id, doi
    Ingest->>Src: force true
    Src->>DB: write source_record_status
    Ingest->>Ft: force true
    Ft->>DB: write full_text_status
    alt full text succeeded
      Ingest->>Brief: force true
      alt brief succeeded
        Ingest->>Eval: force true
      end
    end
  end
  UI->>DB: poll enum columns for progress
```

| Type | Who returns it | When | What the caller does with it |
| --- | --- | --- | --- |
| `PaperIngestEnqueueResult` | `enqueue_ingest_papers` ([Paper archiving](2.2.1-paper-archiving.md)) | Once per archive result | UI stores it so it does not submit duplicate Prefect runs on every Streamlit rerun. It is **not** progress truth. |
| `FulfillPapersMetadataEnqueueResult` | `enqueue_fulfill_papers_metadata` | Ops / Prefect console helper | Not used by Streamlit. |
...
IngestPaperResult
  ...
  evaluation: EvaluatePaperBriefResult | None
```

`brief` is `None` when full text is not `succeeded` after the force full-text step. `evaluation` is `None` when the brief is not `succeeded` after the force brief step (including when brief is skipped). Nested result types live in generate-paper-brief and paper-brief-evaluation schemas; `IngestPaperResult` keeps those fields untyped so the schema modules do not import each other. Do not store Prefect run ids on these types for UI progress.

## Durable `Paper` extensions

Archiving fields remain as in [Paper archiving](2.2.1-paper-archiving.md). This step **adds** status columns and payload columns populated only by the inform flows.
...
`full_text_status = succeeded` means `full_text_plain` holds **usable** article body text: `strip()` of the Cloud `.txt` body, that stripped value is not empty, and the body is not a PMC license-block stub (it does not contain `do not permit archiving in PMC`, case-insensitive). Author manuscript with `is_open_access=false` and usable body text is still `succeeded`.

Empty string, spaces-only, or newline-only body is **not** usable. Treat it as no `.txt`: `full_text_status = unavailable`; leave `full_text_plain` null (clear a previously stored body on this path, including `force` regenerate). Do not store `''` or whitespace.

A **license-block stub** (non-empty body that contains `do not permit archiving in PMC`) is also **not** usable for briefing: set `full_text_status = unavailable`, but **persist** `strip()` of that `.txt` in `full_text_plain` for debug, and write the other Cloud enrichment columns from that hit (`pmcid_version`, `is_open_access`, URLs). When the body is usable, persist `strip()` of the body (remove leading and trailing whitespace).
...
[Generate paper brief](2.2.3-generate-paper-brief.md) requires `full_text_status = succeeded` and grounds the brief on `full_text_plain` only. Full source metadata remains on `Paper` in `source_record` for other tasks.
...
| Rule | Behavior |
| --- | --- |
| Eligibility | PMCID present on the paper **and** Cloud has an article version for that PMCID (Open Access Subset **or** Author Manuscript) **and** Cloud exposes a **usable** body `.txt` (`strip()` not empty, not a license-block stub). |
...
| PMCID, Cloud `.txt` present, license-block stub | `full_text_status = unavailable`. Persist `strip()` of the stub in `full_text_plain` and write enrichment columns from that Cloud hit. |
...
| `full_text_plain` | Text, nullable | `strip()` of the Cloud `.txt` body (plain text extracted from JATS XML). Required for `full_text_status = succeeded` (must be usable: not empty, not a license-block stub). A non-empty license-block stub may still be stored when status is `unavailable` (debug). Never store blank or whitespace-only text. |
...
`indexing.keywords` is the v1 source for [Papers search](papers-search.md). Do not promote that list to a typed query column here. [Paper indexing](2.2.5-paper-indexing.md) generates `keywords_tsv` from that JSON path.
...
| `not_started`, PMCID present, Cloud `.txt` is a license-block stub | `full_text_status = unavailable`. Persist `strip()` of the stub in `full_text_plain` and write enrichment columns. |
...
Leaf flows and `fulfill_paper_metadata` are **idempotent by default** (skip rules above). The documented exception is `ingest_paper`.

## ingest_paper orchestrator

`ingest_paper` is the **only** path that may unfreeze `succeeded` and retry `failed` / `unavailable`.

The user starts one run from [Paper archiving](2.2.1-paper-archiving.md): auto-enqueue for selected papers, or a per-paper **Regenerate** button when both aspects are terminal. The button submits `ingest_paper` (fire-and-forget). Streamlit does not call EFetch, Cloud, or the LLM.

Show the button only when both `source_record_status` and `full_text_status` are terminal (`succeeded` / `failed` / `unavailable`). Do not show it while either aspect is `not_started` (avoids two writers on one paper).

Order (Prefect **subflows**; Streamlit submits only `ingest_paper`):

1. `inform_source_record(paper_id, doi, force=True)` (re-fetch; overwrite payload; set status from the new attempt).
2. `inform_full_text(paper_id, doi, force=True)` (re-try Cloud even after previous `unavailable` / `failed` / `succeeded`).
3. If `full_text_status = succeeded`, call `create_paper_brief(paper_id, doi, force=True)` — contract: [Generate paper brief](2.2.3-generate-paper-brief.md). If full text is not `succeeded`, do not draft or rewrite a brief.
4. If the brief `status` is `succeeded`, call `evaluate_paper_brief(paper_id, doi, force=True)` — contract: [Paper brief evaluation](2.2.4-paper-brief-evaluation.md). If the brief is not `succeeded`, skip evaluation.

Leaf flow signatures are `(paper_id, doi, force=False)` for `inform_source_record`, `inform_full_text`, `create_paper_brief`, and `evaluate_paper_brief`. `ingest_paper` stays `(paper_id, doi)` and always passes `force=True` into those subflows. Streamlit never submits `inform_*`. Served leaf deployments keep default `force=False`, so a standalone Prefect UI run stays idempotent.

## Streamlit UI (v1)

There is **no** dedicated Streamlit page for this step. Progress, auto-enqueue of `ingest_paper`, and the **Regenerate** button live on [Paper archiving](2.2.1-paper-archiving.md).

Do **not** register `fulfill_papers_metadata` in `build_app_pages()`.

Streamlit is presentation only ([technology-stack.md](../technology-stack.md)). Heavy work runs in Prefect. Progress polls durable `Paper` (and `PaperBrief`) enum columns. No Prefect run ids required for the UI contract.

## Workflow navigation

- **Entry / exit / session keys:** [Paper archiving](2.2.1-paper-archiving.md).
- **Input to jobs:** one `paper_id` (and `doi` for the run name) per Prefect run.

## Orchestration boundary

| Responsibility | Owner |
| --- | --- |
| Create/reuse bibliographic `Paper`; ingest auto-enqueue selection; progress UI | [Paper archiving](2.2.1-paper-archiving.md) |
| `PaperAspectStatus` enum; `source_record_status` / `full_text_status`; `fulfill_paper_metadata`; `ingest_paper` steps 1–2 | This document |
...
| Domain aspect inform helpers | `paper_reviewer.topic_scope.fulfill_papers_metadata` (no domain `ingest_paper`; the Prefect parent sequences subflows) |
| Prefect flows | `paper_reviewer.flows` (`inform_source_record`, `inform_full_text`, `fulfill_paper_metadata`, `ingest_paper` parent) |
...
| Paper brief drafting / `ingest_paper` step 3 | [Generate paper brief](2.2.3-generate-paper-brief.md) |
| Paper brief evaluation / `ingest_paper` step 4 | [Paper brief evaluation](2.2.4-paper-brief-evaluation.md) |
| Topic brief generation | [Topic brief generation](4-topic-brief-generation.md) (not this document) |

This document is the **behavior contract** for domain logic and the inform Prefect jobs. Implementation follows [tdd.md](../tdd.md).
...
- PMCID + license-block stub `.txt` → `unavailable`; persist stripped stub in `full_text_plain` and enrichment columns.
...
- Parent flow calls `inform_source_record`, `inform_full_text`, and (when full text succeeded) `create_paper_brief` then (when the brief succeeded) `evaluate_paper_brief` as subflows with `force=True`.
...
- Force full text still `unavailable` → `brief` is `None`; `evaluation` is `None`; existing brief row unchanged.
- Brief not `succeeded` after step 3 → skip `evaluate_paper_brief`; `evaluation` is `None`.

**UI slice** (no Streamlit widget assertions per [tdd.md](../tdd.md)):

- Progress helpers live with Paper archiving (`tests/ui/test_paper_archiving.py`). `tests/ui/test_navigation.py` must **not** register a Fulfill papers metadata page.
- Pure helpers for status → display label and `may_submit_ingest_paper` unit-tested without Streamlit when extracted.
...
- Auto-retry `failed` / `unavailable` or unfreeze `succeeded` except via `ingest_paper`.
- Run EFetch or PMC Cloud calls inside Streamlit.
- Draft the topic brief (phase 4 Topic brief generation).
- Store Prefect run ids for UI progress (DB columns only).

## Future work

**Rich authors (separate job after brief creation):** Register authors as full entities (structured names, affiliations, ORCID when present) and link related papers. Keep flat `authors: list[str]` on `Paper` until that spec exists. That job is an additional **aspect** (new enum + flow) after [Generate paper brief](2.2.3-generate-paper-brief.md), not part of v1 `inform_source_record`.
