...
| **`regenerate_paper`** | Parent Prefect flow: may unfreeze `succeeded` and retry `failed` / `unavailable`, then rewrite the paper brief. Calls `inform_source_record`, `inform_full_text`, and (when full text succeeded) `create_paper_brief` as subflows with `force=true`. Auto-enqueued from Paper archiving for selected papers; also started from a per-paper **Regenerate** button on that page. See [Full regenerate orchestrator](#full-regenerate-orchestrator). |
...
- `inform_source_record` / `inform_full_text` / `fulfill_paper_metadata` (default skip) and `regenerate_paper` (always force).
...
- `regenerate_paper` as the only force path (auto-enqueue selection and **Regenerate** button live on Paper archiving).
...
- Creating **paper briefs** (`PaperBrief`) inside `inform_*` — owned by [Generate paper brief](2.2.3-generate-paper-brief.md). `regenerate_paper` may call `create_paper_brief`.
- A dedicated Streamlit **page** for this step or for `regenerate_paper`.
...
- Auto-retry of `failed` or `unavailable` except via `regenerate_paper`.
- Overwrite of `succeeded` except via `regenerate_paper`.
...
1. **Paper archiving** yields `PaperArchivingResult.papers` (create or reuse) and enqueues `regenerate_paper` for selected papers — see [Paper archiving](2.2.1-paper-archiving.md).
...
3. **Generate paper brief** drafts a global `PaperBrief` only when `full_text_status` is `succeeded` — see [Generate paper brief](2.2.3-generate-paper-brief.md). After archive, that call is `regenerate_paper` step 3.
...
These rules apply when an operator submits `fulfill_paper_metadata` (Prefect console / ops). Streamlit auto-enqueue uses `regenerate_paper` selection in [Paper archiving](2.2.1-paper-archiving.md) instead.
...
DOI on flow parameters is for UI/search and the flow run name (`flow_run_name="{doi}"` on the `@flow` decorator, including subflows from `regenerate_paper`); durable work keys off `paper_id`.
...
| `regenerate_paper` | Parent Prefect flow. Always forces. Calls leaf flows as subflows. See [Full regenerate orchestrator](#full-regenerate-orchestrator). |

| Rule | Behavior |
| --- | --- |
| Independent aspect flows | Each aspect has its own flow. `regenerate_paper` calls them as Prefect subflows. `fulfill_paper_metadata` sequences domain helpers with default skip. |
...
sequenceDiagram
  ...
  participant Enq as enqueue_regenerate_papers
  participant Pref as Prefect
  participant Regen as regenerate_paper
...
| --- | --- | --- | --- |
| `PaperIngestEnqueueResult` | `enqueue_regenerate_papers` ([Paper archiving](2.2.1-paper-archiving.md)) | Once per archive result | UI stores it so it does not submit duplicate Prefect runs on every Streamlit rerun. It is **not** progress truth. |
...
The user starts one run from [Paper archiving](2.2.1-paper-archiving.md): auto-enqueue for selected papers, or a per-paper **Regenerate** button when both aspects are terminal. The button submits `regenerate_paper` (fire-and-forget). Streamlit does not call EFetch, Cloud, or the LLM.
...
Order (Prefect **subflows**; Streamlit submits only `regenerate_paper`):
...
| `PaperAspectStatus` enum; `source_record_status` / `full_text_status`; `fulfill_paper_metadata`; `regenerate_paper` steps 1–2 | This document |
...
| Domain aspect inform helpers | `paper_reviewer.topic_scope.fulfill_papers_metadata` (no domain `regenerate_paper`; the Prefect parent sequences subflows) |
| Prefect flows | `paper_reviewer.flows` (`inform_source_record`, `inform_full_text`, `fulfill_paper_metadata`, `regenerate_paper` parent) |
...
| Paper brief drafting / `regenerate_paper` step 3 | [Generate paper brief](2.2.3-generate-paper-brief.md) |
...
- Auto-retry `failed` / `unavailable` or unfreeze `succeeded` except via `regenerate_paper`.
