...
- If you analyze the same `TopicScope` again, **replace** the facet rows for it.
- Domain helper `run_topic_analysis(session, topic_scope)` that analyzes and persists.
- Dedicated Streamlit **Topic analysis** page: auto-run when the scope has no facet rows; show stored facets otherwise; **Analyze again** replaces rows.
...
| `topic_scope` | Yes (`run_topic_analysis`) | The `TopicScope` whose statement to analyze and whose facet rows to replace. |
...
| Replace | Delete existing facet rows for that `topic_scope_id`, then insert the new rows. Do not leave duplicates. |
...
If you run Topic analysis again for the same `TopicScope`, replace the facet rows. First remove the old rows. Then write the new rows.
...
1. **Prerequisites** — Require `topic_scope_public_id` in the URL ([ui-style.md](../ui-style.md#topic-scope-public-id-in-the-url)). Load the `TopicScope` from the database. If the id or row is missing: empty state and a page_link to **Topic intake**. Do not run analysis.
...
3. **Display** — If facet rows exist: show each facet (`label`, optional `intent`, `concepts`).
4. **Analyze again** — Secondary button ([ui-style.md](../ui-style.md)). On click: run `run_topic_analysis` again (replace rows); clear later-step session caches (related-paper search, triage, archiving, fulfill enqueue, generate-brief enqueue, …) so later pages do not keep stale results — cascade in [Fulfill papers metadata](06-fulfill-papers-metadata.md).
5. **Exit** — After facets exist: `st.page_link` to **Topic scope** (pass `topic_scope_public_id`). Do **not** `st.switch_page` to the hub. Do **not** call `search_related_papers`. Do **not** link directly to a phase landing from this page.
...
1. Require `topic_scope_public_id`. Load `TopicScope` and its facet rows from the database.
2. Missing id/row → empty state + page_link to **Topic intake**.
...
4. When facets exist, render in this order:
   ...
   3. Three phase **page_links** (navigate only; pass the query id): **Paper ingestion**, **Paper search**, **Topic brief**.
...
| Visit with existing facet rows | Show stored facets. Do not auto-run. |
| Analyze again | Replace facet rows; clear later session caches; show new facets. |
...
- Persist / reload / re-analysis: insert rows; reload `TopicAnalysisResult`; second `run_topic_analysis` replaces rows (no duplicates).
...
- UI slice (no Streamlit widget assertions per [tdd.md](../tdd.md)): pages registered with keys `topic_analysis` and `topic_scope`, titles **Topic analysis** / **Topic scope**, url paths `topic-analysis` / `topic-scope`, `in_sidebar` false.
