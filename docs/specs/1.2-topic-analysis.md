...
The contract is `paper_reviewer.schemas.topic_brief_generation.topic_analysis.TopicAnalysisResult`, which holds `facets: list[TopicFacet]`. Search external sources uses the same models.
...
- Implement References selection or Topic brief drafting on the hub (those landings are shells — [References selection](3-references-selection.md), [Topic brief](4-topic-brief.md)).
