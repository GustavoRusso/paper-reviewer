...
This document is the specification for **phase 1, step 2** of the Topic scope workflow in [README.md](../../README.md).
...
A **Topic scope workflow** is the four-phase workflow in [README.md](../../README.md), run on one `TopicScope`. This document specifies phase 1 step 2 (Topic analysis) and the **Topic scope hub** (phase 1 result UI) for that scope.

`SearchCriteria` conversion and search external sources orchestration are out of scope here — see [search external sources](2.1-search-external-sources.md). Topic intake (create `TopicScope`) is [Topic intake](1.1-topic-intake.md).
...
- After facets exist: `st.page_link` to the **Topic scope hub**. Do not run search external sources here. Do not `switch_page` to the hub.
- Dedicated **Topic scope hub** page: statement + facets + links to External sources ingestion, References selection, and Topic brief.
...
- `SearchCriteria` / `source_overrides` (owned by [search external sources](2.1-search-external-sources.md)).
- Running search external sources, paper archiving, or other ingest steps on the analysis or hub pages.
...
flowchart TB
  ...
  facets[(topic_facets)]
  hub[Topic scope hub]
  ingest[External sources ingestion]
  referencesSelection[References selection]
  topicBrief[Topic brief generation]
  home -->|page_link| hub
...
3. The **hub** shows the statement and facets, then the user chooses a later phase. Search external sources is the first ingest step when the user opens [External sources ingestion](2-external-sources-ingestion.md), not an automatic next step after analysis.
...
The contract is `paper_reviewer.schemas.topic_scope.topic_analysis.TopicAnalysisResult`, which holds `facets: list[TopicFacet]`. Search external sources uses the same models.

This step makes a **`TopicAnalysisResult`**. Downstream search envelope types are owned by [search external sources](2.1-search-external-sources.md).
...
Package path: `paper_reviewer.topic_scope.topic_analysis`.
...
4. **Exit** — After facets exist: `st.page_link` to **Topic scope** (pass `topic_scope_key`). Do **not** `st.switch_page` to the hub. Do **not** call `search_external_sources`. Do **not** link directly to a phase landing from this page.
...
Phase 1 result UI. This page does not start ingest, References selection, or topic-brief drafting.
...
4. When facets exist, render in this order:
   ...
   3. Three phase **page_links** (navigate only; pass `topic_scope_key`): **External sources ingestion**, **References selection**, **Topic brief generation**.
5. The user may open any phase without completing the others. Do not add gates between those links.

Phase landing contracts: [External sources ingestion](2-external-sources-ingestion.md), [References selection](3-references-selection.md), [Topic brief generation](4-topic-brief-generation.md).
...
Package path for the analyzer: `paper_reviewer.topic_scope.topic_analysis` — see [project-structure.md](../project-structure.md).
...
| Search external sources / `SearchCriteria` | [search external sources](2.1-search-external-sources.md) |
...
- Search external sources tests inject a `TopicAnalysisResult` and do not need this step’s analyzer — see [search external sources](2.1-search-external-sources.md).
...
- Make PubMed `source_overrides` or MeSH-specific markup (see [external-sources/pubmed.md](external-sources/pubmed.md)).
...
- Run search external sources or `switch_page` to the hub from the analysis page.
- Add an Analyze again control or any other way to re-run Topic analysis for an existing Topic scope.
- Implement Add reference attach or Topic brief generation on the hub (those stay on their phase pages — [Add reference](3.2-add-reference.md), [Topic brief generation](4-topic-brief-generation.md)).
