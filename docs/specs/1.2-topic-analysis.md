...
In this step, the system extracts key data from a **topic statement** as **topic facets**, writes those facets on the **`TopicScope`**, and then lets the user open the **Topic scope hub** to choose a later phase — [Topic scope hub](topic-scope-hub.md).
...
| **Topic scope hub** | Phase 1 result UI. Page contract: [Topic scope hub](topic-scope-hub.md). |

## Topic scope workflow

A **Topic scope workflow** is the four-phase workflow in [README.md](../../README.md), run on one `TopicScope`. This document specifies phase 1 step 2 (Topic analysis) for that scope. The **Topic scope hub** is [Topic scope hub](topic-scope-hub.md).
...
- After facets exist: `st.page_link` to the **Topic scope hub**. Do not run search external sources here. Do not `switch_page` to the hub. Hub page: [Topic scope hub](topic-scope-hub.md).
...
- Running search external sources, paper archiving, or other ingest steps on the analysis page.
- The Topic scope hub page ([Topic scope hub](topic-scope-hub.md)).
...
3. The user opens the **hub** to choose a later phase — [Topic scope hub](topic-scope-hub.md). Search external sources is not an automatic next step after analysis.
...
4. **Exit** — After facets exist: `st.page_link` to **Topic scope** (pass `topic_scope_key`). Do **not** `st.switch_page` to the hub. Do **not** call `search_external_sources`. Do **not** link directly to a phase landing from this page. Hub page: [Topic scope hub](topic-scope-hub.md).
...
| Topic analysis page | `paper_reviewer.ui.topic_analysis` |
| Topic scope hub | [Topic scope hub](topic-scope-hub.md) |
...
- UI slice (no Streamlit widget assertions per [tdd.md](../tdd.md)): page registered with key `topic_analysis`, title **Topic analysis**, url path `topic-analysis`, `in_sidebar` false. Hub page tests: [Topic scope hub](topic-scope-hub.md).
