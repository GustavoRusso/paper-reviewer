...
| `OPENAI_BASE_URL` | `http://localhost:11434/v1` | `prefect-worker`, `notebooks` | Ollama OpenAI-compatible API. Empty uses the public OpenAI API. From Compose, `localhost` / `127.0.0.1` is rewritten to `host.docker.internal` so a host service is reachable. When set, the job sends `max_tokens=4096` and `reasoning_effort=none` (so thinking models write the answer into `content`) and clips extracted scientific sections to 8000 characters |
| `OPENAI_MODEL` | `llama3.1:8b` | `prefect-worker`, `notebooks` | Chat model id. Empty uses the public API default (`gpt-4o-mini`). Required when `OPENAI_BASE_URL` is set. Local Ollama also pulls `gemma4:e4b`; set this to that tag to use it. Notebooks: [paper-brief-evaluation-offline.md](specs/paper-brief-evaluation-offline.md#runtime) |
...
- **`ollama-pull`** — one-shot `ollama pull llama3.1:8b` then `ollama pull gemma4:e4b` after `ollama` is healthy (Compose profile `app`; started by `just up` before `prefect-worker`). Idempotent when both models are already present. First run downloads several GB and can take several minutes. Manual re-run: `just pull-model`.
...
The default `.env.example` is preconfigured for local Ollama (`OPENAI_BASE_URL=http://localhost:11434/v1`, `OPENAI_MODEL=llama3.1:8b`). `just up` starts Ollama and automatically pulls `llama3.1:8b` and `gemma4:e4b` (first startup downloads several GB). Set `OPENAI_MODEL=gemma4:e4b` to use Gemma. Confirm the models are ready: `curl http://localhost:11434/v1/models`. To re-pull both models: `just pull-model`.
