...
- **Paper archiving** — Paper Ingestion substep that creates a `Paper` in this system from each candidate, or reuses an existing `Paper` when that article is already stored. The same page then runs ingest (source record, full text, paper brief) for new papers. Spec: [docs/specs/2.2.1-paper-archiving.md](docs/specs/2.2.1-paper-archiving.md).
...
- **Paper brief evaluation** — Paper Ingestion substep that scores a succeeded **paper brief** with an LLM-as-judge against the paper’s full text. Spec: [docs/specs/2.2.4-paper-brief-evaluation.md](docs/specs/2.2.4-paper-brief-evaluation.md) (docs contract locked; not implemented).
...
- **2.2 Paper Ingestion** — For each found paper, the assistant runs this ingest process (group index: [docs/specs/2.2-paper-ingestion.md](docs/specs/2.2-paper-ingestion.md)):
  ...
  - **2.2.4 Paper brief evaluation** — Scores a succeeded paper brief with an LLM-as-judge (four G-Eval scores on the whole brief; overall mean on `PaperBrief`). Spec: [docs/specs/2.2.4-paper-brief-evaluation.md](docs/specs/2.2.4-paper-brief-evaluation.md) (docs contract locked; not implemented).
