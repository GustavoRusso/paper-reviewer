...
`just up` starts **Ollama** and pulls `gemma4:e4b` (first startup downloads several GB). The `.env.example` settings work as-is:
...
Confirm the model is ready: `curl http://localhost:11434/v1/models`. Re-pull: `just pull-model`. Details: [docs/local-development.md](docs/local-development.md#local-llm-ollama). Quality notes: [Paper brief quality](#paper-brief-quality-offline-evaluation).
...
Warning: this project uses LLMs heavily (paper briefs, evaluation, topic briefs). The public OpenAI API can be **very expensive**.
...
| LLM evaluation                       | [docs/specs/2.2.4-paper-brief-evaluation.mddocs/specs/paper-brief-evaluation-offline.md](docs/specs/2.2.4-paper-brief-evaluation.md)                | Run an online evaluation on each generated paper brief. Run an offline evaluation using in-app G-Eval + offline judge runs. Offline results: [Paper brief quality](#paper-brief-quality-offline-evaluation)                            |
| Interface                            | [docs/ui-style.mddocs/specs/topic-scope-hub.md](docs/ui-style.md)                                                                                   | Streamlit UI; default URL in [Services](#services)                                                                                                                                                                                     |
| Ingestion pipeline                   | [docs/specs/2.1-search-external-sources.mddocs/specs/2.2-paper-ingestion.md](docs/specs/2.1-search-external-sources.md)                             | dlt extract + Prefect `ingest_paper`; stack: [technology-stack.md](docs/technology-stack.md)                                                                                                                                           |
...
| Containerization                     | [docs/host-requirements.mddocs/local-development.md](docs/host-requirements.md)                                                                     | Full app stack in `compose.yml` via `just`                                                                                                                                                                                             |
