"""Landing page helpers: Topic scope list formatting and empty copy."""
...
def test_empty_topic_scopes_message() -> None:
    assert EMPTY_TOPIC_SCOPES_MESSAGE == "No Topic scopes yet."
...
def test_format_topic_scope_created_at_iso_utc() -> None:
    ...
    assert format_topic_scope_created_at(created) == "2026-08-12T11:30:00+00:00"


def test_format_topic_scope_reference_caption() -> None:
    topic_scope_key = uuid.UUID("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee")

    assert (
        format_topic_scope_reference_caption(topic_scope_key)
        == "Reference id: `aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`"
    )


def test_format_topic_scope_list_caption() -> None:
    topic_scope_key = uuid.UUID("11111111-2222-3333-4444-555555555555")
    created = datetime(2026, 8, 11, 12, 0, 0, tzinfo=UTC)

    assert format_topic_scope_list_caption(
