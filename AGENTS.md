...
| [docs/specs/paper-briefs.md](docs/specs/paper-briefs.md)                 | Paper briefs (step 6): idempotent Prefect `inform_paper_from_source` (EFetch) then `create_paper_brief` (LLM); durable `source_informed_at`; progress UI | When implementing or changing source-inform, `PaperBrief`, brief Prefect jobs, or the Paper briefs progress page |
