...
| [docs/ui-style.md](docs/ui-style.md)                                     | Web UI semantic style: links navigate only; buttons mutate; one look per intent (primary / default / cancel / danger) and Streamlit widget mapping | When adding or changing Streamlit buttons, page links, form submits, or empty-state CTAs; when writing UI sections of step specs |
...
| [docs/specs/06-fulfill-papers-metadata.md](docs/specs/06-fulfill-papers-metadata.md) | Fulfill papers metadata (step 6): Prefect `inform_paper_from_source` (EFetch via dlt); optional PMC Cloud `full_text_plain` + `open_access_pdf_url` / `pmc_article_url`; `source_record` JSONB + typed promotes; `source_informed_at` / error message; progress UI polls DB | When implementing or changing source-inform, `Paper` EFetch/Cloud storage, or the Fulfill papers metadata progress page |
| [docs/specs/07-generate-paper-brief.md](docs/specs/07-generate-paper-brief.md) | Generate paper brief (step 7): idempotent Prefect `create_paper_brief` (LLM); prefer `full_text_plain` when present else abstract; dedicated progress UI | When implementing or changing `PaperBrief`, brief Prefect jobs, or the Generate paper brief progress page |
| [docs/specs/paper-sources/pubmed.md](docs/specs/paper-sources/pubmed.md) | PubMed Entrez mapping to `PaperCandidate`, EFetch dlt resource for source-inform, NCBI API key / rate limits                                                                  | When implementing or changing PubMed search, Entrez queries, PubMed → candidate mapping, EFetch mapping, or NCBI ops                                                                                                 |
