...
| [docs/specs/paper-brief-evaluation-offline.md](docs/specs/paper-brief-evaluation-offline.md) | Offline paper-brief evaluation: freeze a full-text corpus, generate briefs on disk, and score them with the same judge as 2.2.4; corpus/results under `data/` (tracked, not deployed). Step 1 and `just notebooks` implemented; steps 2–3 not yet | When implementing or changing the offline eval notebooks, corpus layout, or file-only judge runs |
