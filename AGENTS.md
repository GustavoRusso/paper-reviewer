...
- `/docs/specs` — Feature and workflow specs. Phase 1 (Topic definition) uses a dotted `P.S-name.md` prefix matching [README.md](README.md) (e.g. `1.1-topic-intake.md`, `1.2-topic-analysis.md`). Phase landings use `2-external-sources-ingestion.md`, `3-references-selection.md`, `4-topic-brief.md`. Phase 2 uses `2.1-search-external-sources.md`, docs-only group `2.2-paper-ingestion.md`, and leaf specs `2.2.1`–`2.2.4`. Phase 3 uses `3.1-show-references.md` and `3.2-add-reference.md`. Shared local **Papers search** lives at `papers-search.md` (no step number). External-source-specific docs live under `docs/specs/external-sources/` (no step number).
...
| [docs/specs/2.2-paper-ingestion.md](docs/specs/2.2-paper-ingestion.md) | Paper Ingestion (phase 2 step 2, docs-only group): index of archive / fulfill / brief / indexing leaf steps; no Streamlit page in v1 | When looking up the Paper Ingestion step group or its substep list |
| [docs/specs/2.2.1-paper-archiving.md](docs/specs/2.2.1-paper-archiving.md) | Paper archiving (2.2.1): dedicated Streamlit page auto-runs create-or-reuse `Paper` from search external sources `candidates`; DOI uppercase; fail-soft skips; no EFetch | When implementing or changing paper create/reuse from candidates, `Paper` fields, archiving UI, or identity rules |
| [docs/specs/2.2.2-fulfill-papers-metadata.md](docs/specs/2.2.2-fulfill-papers-metadata.md) | Fulfill papers metadata (2.2.2): `PaperAspectStatus` on `Paper` (`source_record_status`, `full_text_status`); flows `inform_source_record` / `inform_full_text` / page `fulfill_paper_metadata`; `regenerate_paper` is the only force path | When implementing or changing source-record or full-text fulfill, `Paper` status enums, or the Fulfill papers metadata progress page |
| [docs/specs/2.2.3-generate-paper-brief.md](docs/specs/2.2.3-generate-paper-brief.md) | Generate paper brief (2.2.3): global 1:1 `PaperBrief`; gated on full text `succeeded`; topic-agnostic content; page enqueues `create_paper_brief` only. Brief section list and LLM prompt: [src/paper_reviewer/topic_brief_generation/generate_paper_brief/paper_brief_template.md](src/paper_reviewer/topic_brief_generation/generate_paper_brief/paper_brief_template.md) | When implementing or changing `PaperBrief`, brief Prefect jobs, the brief template, or the Generate paper brief progress page |
| [docs/specs/2.2.4-paper-indexing.md](docs/specs/2.2.4-paper-indexing.md) | Paper indexing (2.2.4): stub; index ingested papers for Papers search (not built) | When implementing or changing paper indexing for local Papers search |
| [docs/specs/3-references-selection.md](docs/specs/3-references-selection.md) | References selection phase landing (v1 shell): links to Show references and Add reference | When implementing or changing the References selection landing |
| [docs/specs/3.1-show-references.md](docs/specs/3.1-show-references.md) | Show references (3.1): list References already attached to the Topic scope (not built) | When implementing or changing the Show references page or Reference list UX |
| [docs/specs/3.2-add-reference.md](docs/specs/3.2-add-reference.md) | Add reference (3.2): Papers search results + add one / add all as References (not built) | When implementing or changing Add reference attach UX |
| [docs/specs/papers-search.md](docs/specs/papers-search.md) | Papers search capability: apply topic facets to the local ingested paper store/index (not built) | When implementing or changing local Papers search used by Add reference |
