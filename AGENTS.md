...
- `/docs/specs` — Feature and workflow specs. Workflow-step specs use a zero-padded step number prefix matching [README.md](README.md) (e.g. `06-fulfill-papers-metadata.md` for step 6). Paper-source-specific docs live under `docs/specs/paper-sources/` (no step number).
...
| [docs/ui-style.md](docs/ui-style.md)                                     | Web UI semantic style: links navigate only; buttons mutate; one look per intent (primary / default / cancel / danger); Streamlit widget mapping; theme colours in `.streamlit/config.toml` | When adding or changing Streamlit buttons, page links, form submits, empty-state CTAs, or theme colours; when writing UI sections of step specs |
...
| [docs/specs/02-topic-analysis.md](docs/specs/02-topic-analysis.md)       | Topic analysis (step 2 of a Topic brief generation): scispaCy NER → in-memory `TopicAnalysisResult` (`TopicFacet` list); facet DB persistence deferred                                                                          | When implementing or changing topic analysis or facet extraction                                                                                                                             |
...
| [docs/plans/paper-aspect-status.md](docs/plans/paper-aspect-status.md) | Step-by-step implementation plan for paper aspect statuses, split fulfill flows, global paper brief, and force regenerate | When applying the spec 06/07 status contract in code; each plan step is one reviewable commit |
