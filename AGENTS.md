...
- `/docs/specs` — Feature and workflow specs. Phase 1 (Topic definition) uses a dotted `P.S-name.md` prefix matching [README.md](README.md) (e.g. `1.1-topic-intake.md`, `1.2-topic-analysis.md`). Phase landings use `2-external-sources-ingestion.md` and `4-topic-brief-generation.md`. Phase 3 overview `3-references-selection.md` is docs-only (shared chrome; no Streamlit landing; hub opens Show references). Phase 2 uses `2.1-search-external-sources.md`, docs-only group `2.2-paper-ingestion.md`, and leaf specs `2.2.1`–`2.2.4`. Phase 3 uses `3.1-show-references.md` and `3.2-add-reference.md`. Shared local **Papers search** lives at `papers-search.md` (no step number). External-source-specific docs live under `docs/specs/external-sources/` (no step number).
