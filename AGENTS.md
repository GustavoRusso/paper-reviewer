...
| [docs/specs/04-retrieval-triage.md](docs/specs/04-retrieval-triage.md)         | Retrieval triage (step 4): review gate for search candidates; v1 pass-through; confirm stores `retained` and links to the Paper archiving page (clears archive cache on re-confirm) | When implementing or changing triage confirm API, retained-set handoff, or the triage UI page |
...
| [docs/specs/06-fulfill-papers-metadata.md](docs/specs/06-fulfill-papers-metadata.md) | Fulfill papers metadata (step 6): idempotent Prefect `inform_paper_from_source` (EFetch); durable `source_informed_at`; dedicated progress UI | When implementing or changing source-inform, `Paper` EFetch field groups, or the Fulfill papers metadata progress page |
| [docs/specs/07-generate-paper-brief.md](docs/specs/07-generate-paper-brief.md) | Generate paper brief (step 7): idempotent Prefect `create_paper_brief` (LLM); `PaperBrief` model/status/content; dedicated progress UI | When implementing or changing `PaperBrief`, brief Prefect jobs, or the Generate paper brief progress page |
| [docs/specs/paper-sources/pubmed.md](docs/specs/paper-sources/pubmed.md) | PubMed Entrez mapping to `PaperCandidate`, EFetch for source-inform, NCBI API key / rate limits                                                                  | When implementing or changing PubMed search, Entrez queries, PubMed → candidate mapping, EFetch mapping, or NCBI ops                                                                                                 |
