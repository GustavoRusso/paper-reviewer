...
| [docs/specs/06-fulfill-papers-metadata.md](docs/specs/06-fulfill-papers-metadata.md) | Fulfill papers metadata (step 6): Prefect `inform_paper_from_source` (EFetch via dlt); `source_record` JSONB + typed promotes; `source_informed_at` / error message; progress UI polls DB | When implementing or changing source-inform, `Paper` EFetch storage, or the Fulfill papers metadata progress page |
| [docs/specs/07-generate-paper-brief.md](docs/specs/07-generate-paper-brief.md) | Generate paper brief (step 7): idempotent Prefect `create_paper_brief` (LLM); abstract-focused briefs; full metadata stays on `Paper.source_record`; dedicated progress UI | When implementing or changing `PaperBrief`, brief Prefect jobs, or the Generate paper brief progress page |
