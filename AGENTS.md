...
- `/docs/specs` — Feature and workflow specs. Phase 1 (Topic definition) uses a dotted `P.S-name.md` prefix matching [README.md](README.md) (e.g. `1.1-topic-intake.md`, `1.2-topic-analysis.md`). Phase landings use `2-paper-ingestion.md`, `3-paper-search.md`, `4-topic-brief.md`. Phase 2 step 1 uses `2.1-related-paper-search.md`. Remaining ingest-step specs keep a zero-padded linear prefix (`04`–`07`) until those steps are retitled. Paper-source-specific docs live under `docs/specs/paper-sources/` (no step number).
...
| [docs/specs/2.1-related-paper-search.md](docs/specs/2.1-related-paper-search.md) | Related-paper search (phase 2 step 1): URL `topic_scope_key` + DB facets → `search_related_papers`; dlt extract; in-memory `PaperCandidate` merge; session output cache keyed by Topic scope | When implementing or changing search orchestration, criteria input, candidate normalization, or the Related-paper search page |
