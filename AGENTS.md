...
| [docs/local-development.md](docs/local-development.md)                   | `.env` / Compose; host/`just` vs Dev Containers; app vs sandbox; agent shells; tests; Alembic; dltHub MCP; `just notebooks` | First local config after host tools; whenever starting the workspace, Reopen in Container, managing app vs sandbox, migrations, tests, MCP, or offline eval notebooks |
