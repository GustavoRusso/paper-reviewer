...
- `/docs/specs` — Feature and workflow specs. Phase 1 (Topic definition) uses a dotted `P.S-name.md` prefix matching [README.md](README.md) (e.g. `1.1-topic-intake.md`, `1.2-topic-analysis.md`). Phase landings use `2-paper-ingestion.md`, `3-paper-search.md`, `4-topic-brief.md`. Phase 2 step 1 uses `2.1-search-external-sources.md`. Remaining ingest-step specs keep a zero-padded linear prefix (`05`–`07`) until those steps are retitled. External-source-specific docs live under `docs/specs/external-sources/` (no step number).
...
| [docs/specs/2-paper-ingestion.md](docs/specs/2-paper-ingestion.md) | Paper ingestion phase landing: links into search external sources and later ingest pages | When implementing or changing the Paper ingestion landing |
...
| [docs/specs/05-paper-archiving.md](docs/specs/05-paper-archiving.md)           | Paper archiving: dedicated Streamlit page auto-runs create-or-reuse `Paper` from search external sources `candidates`; DOI uppercase; fail-soft skips; no EFetch | When implementing or changing paper create/reuse from candidates, `Paper` fields, archiving UI, or identity rules |
