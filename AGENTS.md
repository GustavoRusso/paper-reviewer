...
- `/docs/specs` — Feature and workflow specs. Workflow-step specs use a zero-padded step number prefix matching [README.md](README.md) (e.g. `06-paper-briefs-generation.md` for step 6). Paper-source-specific docs live under `docs/specs/paper-sources/` (no step number).
...
| [docs/specs/06-paper-briefs-generation.md](docs/specs/06-paper-briefs-generation.md)                 | Paper briefs generation (step 6): idempotent Prefect `inform_paper_from_source` (EFetch) then `create_paper_brief` (LLM); durable `source_informed_at`; progress UI | When implementing or changing source-inform, `PaperBrief`, brief Prefect jobs, or the Paper briefs generation progress page |
