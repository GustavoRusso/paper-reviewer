...
- `/docs/specs` — Feature and workflow specs. Phase 1 (Topic definition) uses a dotted `P.S-name.md` prefix matching [README.md](README.md) (e.g. `1.1-topic-intake.md`, `1.2-topic-analysis.md`). Phase landings use `2-paper-ingestion.md`, `3-paper-search.md`, `4-topic-brief.md`. Older ingest-step specs keep a zero-padded linear prefix (`03`–`07`) until those steps are retitled. Paper-source-specific docs live under `docs/specs/paper-sources/` (no step number).
...
| [docs/technology-stack.md](docs/technology-stack.md)                     | App runtime stack and boundaries: Python, uv, Postgres, dlt extract (load later), scispaCy, SQLAlchemy/Alembic (including no `ON DELETE CASCADE`), Streamlit, Prefect (Compose) | When adding libraries or structuring features across UI, ingest, search, DB, and jobs; when defining FKs or delete behavior |
| [docs/ui-style.md](docs/ui-style.md)                                     | Web UI semantic style: links navigate only; buttons mutate; one look per intent (primary / default / cancel / danger); Streamlit widget mapping; theme colours in `.streamlit/config.toml`; sidebar opt-in | When adding or changing Streamlit buttons, page links, form submits, empty-state CTAs, or theme colours; when writing UI sections of step specs; when adding a page to (or omitting it from) the left sidebar |
| [docs/project-structure.md](docs/project-structure.md)                   | Repo layout, deploy vs local-only paths, `pyproject.toml` placement, package module map (`topic_brief_generation` steps / `schemas` / `models` including global `paper` and `paper_brief` / `ingest` / `flows`) | When adding packages/modules, deciding what Docker images copy, or where to put ORM/UI/workflow-step/ingest/flow code |
...
| [docs/specs/1.2-topic-analysis.md](docs/specs/1.2-topic-analysis.md) | Topic analysis (phase 1 step 2): scispaCy NER → persist `TopicFacet` rows on `TopicScope`; Topic scope hub (statement, facets, independent phase links); Home list links to the hub | When implementing or changing topic analysis, facet persistence, the analysis page, the Topic scope hub, or Home links to the hub |
| [docs/specs/2-paper-ingestion.md](docs/specs/2-paper-ingestion.md) | Paper ingestion phase landing: links into related-paper search and later ingest pages | When implementing or changing the Paper ingestion landing |
| [docs/specs/3-paper-search.md](docs/specs/3-paper-search.md) | Paper search phase landing (v1 shell: local DB search not built) | When implementing or changing the Paper search landing |
...
| [docs/specs/03-related-paper-search.md](docs/specs/03-related-paper-search.md) | Related-paper search: accept `TopicAnalysisResult`, convert internally to `SearchCriteria` when needed, dlt extract across sources, in-memory `PaperCandidate` merge; dedicated search page | When implementing or changing search orchestration, criteria input, candidate normalization, or the Related-paper search page |
| [docs/specs/04-retrieval-triage.md](docs/specs/04-retrieval-triage.md) | Retrieval triage (step 4): review gate; v1 pass-through; confirm stores `retained`, clears archiving **and later-step** session caches, links to Paper archiving | When implementing or changing triage confirm API, retained-set handoff, or the triage UI page |
| [docs/specs/05-paper-archiving.md](docs/specs/05-paper-archiving.md)           | Paper archiving (step 5): dedicated Streamlit page auto-runs create-or-reuse `Paper` from triage `retained`; DOI uppercase; fail-soft skips; no EFetch | When implementing or changing paper create/reuse from candidates, `Paper` fields, archiving UI, or identity rules |
| [docs/specs/06-fulfill-papers-metadata.md](docs/specs/06-fulfill-papers-metadata.md) | Fulfill papers metadata (step 6): `PaperAspectStatus` on `Paper` (`source_record_status`, `full_text_status`); flows `inform_source_record` / `inform_full_text` / page-6 `fulfill_paper_metadata`; `regenerate_paper` is the only force path | When implementing or changing source-record or full-text fulfill, `Paper` status enums, or the Fulfill papers metadata progress page |
| [docs/specs/07-generate-paper-brief.md](docs/specs/07-generate-paper-brief.md) | Generate paper brief (step 7): global 1:1 `PaperBrief`; gated on full text `succeeded`; topic-agnostic content; page 7 enqueues `create_paper_brief` only. Brief section list and LLM prompt: [src/paper_reviewer/topic_brief_generation/generate_paper_brief/paper_brief_template.md](src/paper_reviewer/topic_brief_generation/generate_paper_brief/paper_brief_template.md) | When implementing or changing `PaperBrief`, brief Prefect jobs, the brief template, or the Generate paper brief progress page |
| [docs/specs/paper-sources/pubmed.md](docs/specs/paper-sources/pubmed.md) | PubMed Entrez mapping to `PaperCandidate`, EFetch for source record, PMC Cloud for full text, NCBI API key / rate limits | When implementing or changing PubMed search, Entrez queries, PubMed → candidate mapping, EFetch mapping, or NCBI ops |
